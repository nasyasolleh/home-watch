...
async function initializeDashboard() {
    try {
        ...
        console.log('API Service available, checking health...');
...
function initProgramsChart(programMentions = []) {
    ...
    if (window.programsChart && typeof window.programsChart.destroy === 'function') {
...
function initSourcesChart(sourceDistribution = []) {
    ...
    if (window.sourcesChart && typeof window.sourcesChart.destroy === 'function') {
...
async function initializeWordCloud() {
    try {
        ...
        // Set up type selector event listener
        const typeSelector = document.getElementById('wordcloudType');
        if (typeSelector) {
            typeSelector.addEventListener('change', function() {
                loadWordCloudData(this.value);
            });
        }
        
        // Load initial word cloud data
        await loadWordCloudData('all');
        
    } catch (error) {
        console.error('Word cloud initialization failed:', error);
        showWordCloudError();
    }
}

async function loadWordCloudData(type = 'all') {
    try {
        const container = document.getElementById('wordcloudContainer');
        if (!container) return;
        ...
        const apiBaseUrl = window.AppConfig?.api?.baseUrl || 'http://localhost:5001';
        const response = await fetch(`${apiBaseUrl}/api/analytics/wordcloud?type=${type}&limit=50`);
        ...
        currentWordCloudData = result.data;
        renderWordCloud(result.data.words);
...
function renderWordCloud(words) {
    const container = document.getElementById('wordcloudContainer');
    if (!container || !words || Object.keys(words).length === 0) {
        showWordCloudError('No data available for word cloud');
        return;
    }
    ...
    svg.setAttribute('class', 'wordcloud-svg');
    ...
    if (wordArray.length === 0) {
        showWordCloudError('No words to display');
        return;
    }
    ...
    const colors = [
        ...
        '#059669', '#10b981', '#065f46', '#047857', '#064e3b',
        '#dc2626', '#ef4444', '#b91c1c', '#991b1b', '#7f1d1d',
        '#7c2d12', '#ea580c', '#c2410c', '#9a3412', '#78350f'
    ];
    
    // Generate positioned words using a simple spiral layout
    const placedWords = [];
    const centerX = 400;
    const centerY = 200;
    ...
    wordArray.forEach((wordData, index) => {
        ...
        textElement.setAttribute('x', Math.max(50, Math.min(750, x)));
        textElement.setAttribute('y', Math.max(30, Math.min(370, y)));
        textElement.setAttribute('font-size', fontSize);
        textElement.setAttribute('fill', color);
        textElement.setAttribute('text-anchor', 'middle');
        textElement.setAttribute('dominant-baseline', 'middle');
        textElement.setAttribute('class', 'wordcloud-word');
        textElement.setAttribute('data-count', wordData.count);
        textElement.textContent = wordData.word;
        
        // Add hover tooltip
        const title = document.createElementNS('http://www.w3.org/2000/svg', 'title');
        title.textContent = `${wordData.word}: ${wordData.count} mentions`;
    ...
    legend.className = 'wordcloud-legend';
    legend.innerHTML = `
        <div>Font size = word frequency</div>
        <div>Total words analyzed: ${currentWordCloudData?.total_texts || 'N/A'}</div>
    `;
    ...
    placedWords.forEach(item => {
        item.element.addEventListener('click', function() {
            showWordDetails(item.word, item.count);
        });
    });
}

function showWordDetails(word, count) {
    // Create a simple modal or alert with word details
    const message = `Word: "${word}"\nMentions: ${count}\nSource: ${currentWordCloudData?.type || 'all data'}`;
    alert(message); // You can replace this with a proper modal
}

function showWordCloudError(message = 'Failed to generate word cloud') {
    const container = document.getElementById('wordcloudContainer');
    if (!container) return;
    ...
    container.innerHTML = `
        <div class="error-state">
            ...
            <div style="color: var(--text-secondary); text-align: center;">
                ...
                <button class="btn btn-sm" onclick="loadWordCloudData('all')" style="margin-top: 1rem;">
