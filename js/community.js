...
class CommunityManager {
  ...
  async loadPosts() {
    ...
    try {
      ...
      try {
        ...
        console.warn("Failed to load from Firestore, using sample data:", error)
        allPosts = this.getSamplePosts()
      }
  ...
  async toggleLike(postId) {
    ...
    try {
      ...
      if (likeBtn) {
        const currentCount = Number.parseInt(likeBtn.textContent.match(/\d+/)[0])
  ...
  async addComment(postId, commentText) {
    ...
    try {
      ...
      // Refresh the post detail if it's open
  ...
  // Updated showPostDetail function with better modal handling
  async showPostDetail(postId) {
    ...
    try {
      ...
      if (!postDoc.exists()) {
        // Fallback to sample data if post not found in Firebase
        const samplePosts = this.getSamplePosts()
                            ...
                            <div class="comments-list" style="max-height: 300px; overflow-y: auto; margin-top: 1.5rem;">
                                ${
                                  comments.length > 0
                                    ? comments
                                        .map((comment) => {
                                          const isAuthor = auth.currentUser && comment.authorId === auth.currentUser.uid
                                          return `
                                        <div class="comment" style="padding: 1rem 0; border-bottom: 1px solid var(--border-color);">
  ...
  // Add this new method to your CommunityManager class:
  ...
  async analyzeBatchSentiment(posts) {
    ...
    try {
      const results = await window.apiService.analyzeSentimentBatch(textsToAnalyze, {
        context: "housing",
  ...
  async createNewPost() {
    ...
    submitBtn.disabled = true
    submitBtn.innerHTML = '<span class="spinner"></span> Analyzing & Posting...'

    try {
      let sentimentData = null
      if (window.apiService) {
        try {
          sentimentData = await window.apiService.analyzeSentiment(`${title} ${content}`, {
            context: "housing",
      ...
      const sentimentInfo = sentimentData ? ` (Sentiment: ${sentimentData.sentiment_label})` : ""
      this.showNotification(`Post created successfully!${sentimentInfo}`, "success")
    } catch (error) {
...
function sharePost(postId) {
  if (navigator.share) {
    ...
    window.communityManager.showNotification("Link copied to clipboard!", "success")
  }
}
